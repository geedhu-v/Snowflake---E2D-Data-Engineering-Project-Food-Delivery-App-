...
-- query external locatiaon stage
select 
...
-- run copy command to load location data. 
...
-- check the copy status using copy history table function.
select *
    from 
        table(information_schema.copy_history(table_name=>'LOCATION', start_time=> dateadd(hours, -1, current_timestamp())));

-- run select statement
select * from stage_sch.location;
select * from stage_sch.location_stm;

-- switch context for clean layer processing.
...
-- check if the count matches or not
select count(*) from clean_sch.restaurant_location;

-- also check with active flag. 
select * from clean_sch.Restaurant_Location
where active_flag = 'No';

-- check if there is data in stream object or not. 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/delta/location/day-02-add-update-17-location.csv t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;
