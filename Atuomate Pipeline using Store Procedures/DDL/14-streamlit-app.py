...
def fetch_kpi_data():
    ...
    SELECT 
        ...
        max_order_value
    FROM sp_pipeline_db.consumption_sch.vw_yearly_revenue_kpis
    ORDER BY year;
    """
...
def fetch_monthly_kpi_data(year):
    ...
    FROM 
    sp_pipeline_db.consumption_sch.vw_monthly_revenue_kpis
    WHERE year = {year}
    ORDER BY month;
    """
    return session.sql(query).collect()
...
def fetch_unique_months(year):
    ...
    SELECT 
        DISTINCT MONTH FROM 
    sp_pipeline_db.consumption_sch.vw_monthly_revenue_by_restaurant 
    WHERE YEAR = {year} 
    ORDER BY MONTH;
    """
    return session.sql(query).collect()
...
with col1:
    st.metric(
        ...
        delta=int(revenue_diff) if revenue_diff is not None else None
    )
...
with col2:
    ...
    st.metric(
        ...
        delta=int(avg_rev_order_diff) if avg_rev_order_diff is not None else None, 
        delta_color="normal"  
    )
    st.metric("Avg Revenue per Item", f"₹{avg_revenue_per_item:,.0f}", delta=int(avg_rev_item_diff) if avg_rev_item_diff is not None else None)

with col3:
    st.metric("Max Order Value", f"₹{max_order_value:,.0f}", delta=int(max_order_diff) if max_order_diff is not None else None)
